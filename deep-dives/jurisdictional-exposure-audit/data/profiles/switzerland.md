...
- **Professional-trader reclassification:** the exemption is **lost** if the taxpayer is deemed a professional securities trader; then gains are ordinary income plus social contributions. **FTA Circular 36 (27 Jul 2012)** sets five safe-harbour criteria (all must hold): ≥6-month holding; annual turnover ≤5× year-start portfolio value; gains not needed to replace lost income; realized gains <50% of net income; derivatives only to hedge own holdings. Failing a safe harbour triggers a facts-and-circumstances review, so reclassification is a judgment call. `Medium` [src-ch-estv-crypto]
- **Wealth tax:** privately held Bitcoin is subject to **cantonal/communal** net-wealth tax (no federal wealth tax), progressive, generally well under ~1%, varying by canton. The exact FTA year-end valuation rate used for Bitcoin is a separate point still to be confirmed against an FTA source. `High` (cantonal taxation) / valuation rate `needs-review` [src-ch-estv-crypto]
...
Switzerland participates in **CRS** (in force 2017; first exchange 2018; 100+ partners). It is moving toward **OECD CARF** implementation; the first-exchange timing should be checked against current Swiss Federal Council / FDF materials (CARF item Medium / needs-review). A 2026 AML reform introduces a central UBO register; FATF's 5th-round evaluation is scheduled 2026–2027. `Reporting: High (CARF date: Medium)` [src-ch-sif-aeoi, src-ch-step-carf]

## 5. Estate and succession

Forced heirship (**Pflichtteil / réserve**) under the Civil Code. The **2023 reform** (in force 1 Jan 2023) reduced descendants' reserved portion from **3/4 to 1/2** of the intestate share, **abolished parents' compulsory portion**, and left the spouse's 1/2 unchanged — enlarging the freely disposable quota. **No federal inheritance/gift tax**; cantonal: spouses/registered partners exempt in **all** cantons; direct descendants exempt in **virtually all cantons except three** — Appenzell Innerrhoden (CHF 300k allowance per child, then ~1%), Vaud (allowances then progressive), and Neuchâtel (nominal 3%, allowances often reduce to near zero). Confirm per canton. Foreign trusts may be recognized under the **Hague Trust Convention**, but mandatory succession, matrimonial-property and other rules can limit outcomes. Default **matrimonial regime is participation in acquisitions** (Errungenschaftsbeteiligung); the property settlement precedes the succession division. `Estate: Medium` [src-ch-baerkarrer-succession, src-ch-chch-inheritance, src-ch-admin-hague-trust]
