...
12. **F17.** Reconcile BIP-360 and BIP-361 — they are separate proposals with separate roles. BIP-360 (P2MR, quantum-resistant address type) is the activation that matters for F17(a). BIP-361 (the contested "Post Quantum Migration and Legacy Signature Sunset" proposal) is contested and is *not* required for F17 to stay INTACT. Source URL updated to point to the BIP-360 repository, not the BitVM bridge paper.
...
- **BIP-361** — *"Post Quantum Migration and Legacy Signature Sunset"* (Draft/Informational; Lopp et al.; assigned Feb 11, 2026). **Two phases:** Phase A restricts new sends to quantum-vulnerable address types; Phase B tightens legacy ECDSA/Schnorr spending to require a quantum-safe rescue protocol after an announced deadline (rescue mechanisms — hardened BIP-32 derivation, ZK-STARKs, commit-reveal — are part of Phase B; some outputs, notably P2PK, may have no known rescue). Not a bare "freeze," and BIP assignment ≠ consensus/activation/adoption. **Contested. Not required for F17 to stay INTACT and not an F17(a) input.** Voluntary migration only reaches coins with a live, capable owner; the structurally unmovable exposed-key set (lost keys, deceased/inactive owners, esp. Satoshi-era P2PK) cannot migrate, so the residual exposed supply is **not reliably bounded** and its size is **unmeasured** (Deloitte's ~2020 scan and Coinbase's 2026 estimate do not measure the movable-vs-unmovable split).
...
- **(b)** any successful quantum-attack-based theft of BTC from a previously-secure address before end of 2032 — **evaluated by branch** (below), because different populations falsify different claims.

**F17(b) branch structure.** **B1** — theft of *structurally unmovable* exposed-key coins (lost/dormant, esp. Satoshi-era P2PK). Does **not** auto-invoke the unilateral rule; triggers a **mandatory claim-by-claim review**: monetary-cap/issuance (the 21M cap and ledger history stay intact; a large reactivation of inert coins raises effective float → supply/price shock, *not* a fixed-supply-rule failure), ownership-assurance/authorization (control of a legacy output is no longer exclusive to the socially-recognized owner because the attacker can derive the signing key — *not* an immutability failure), governance neutrality, and market confidence. **B2a** — theft from a holder who adopted the best available protection *for the relevant attack class* (P2MR vs long-exposure; a future post-quantum signature vs short-exposure); fires the core-SoV falsifier **only if** attributable to a break of the replacement cryptography itself, and attribution must name what broke (P2MR commitment / classical signature exposed at spend / future PQ scheme / implementation bug). **B2b** — theft from a movable holder who stayed vulnerable despite an available migration path: **user non-migration**, not a cryptography failure. The unilateral-invalidation rule applies to (a) and to a B2a cryptographic break; a B1-only event does not auto-reject the thesis.
