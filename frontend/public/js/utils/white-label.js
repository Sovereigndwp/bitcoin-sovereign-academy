...
export class WhiteLabelManager {
    ...
    getTenant(identifier) {
        ...
        if (!tenant) {
            // Try by domain
            tenant = Array.from(this.tenants.values()).find(t => t.domain === identifier || t.subdomain === identifier);
        }
        return tenant;
    }
    /**
     ...
     */
    detectTenantFromHostname(hostname) {
        return Array.from(this.tenants.values()).find(tenant => {
            return (tenant.domain === hostname ||
...
`;
    ...
    generateEnvFile(tenantId) {
        ...
        if (tenant.integrations.analytics.plausible?.enabled) {
            lines.push(`PLAUSIBLE_DOMAIN=${tenant.integrations.analytics.plausible.domain}`, 'PLAUSIBLE_ENABLED=true');
        }
        if (tenant.integrations.email) {
            lines.push('', '# Email', `EMAIL_PROVIDER=${tenant.integrations.email.provider}`, `EMAIL_FROM=${tenant.integrations.email.fromAddress}`, `EMAIL_FROM_NAME=${tenant.integrations.email.fromName}`);
        }
        return lines.join('\n');
    }
    /**
     ...
     */
    generateTenantHTML(tenantId, pageContent) {
        const tenant = this.getTenant(tenantId);
        if (!tenant)
            return pageContent;
        return `<!DOCTYPE html>
...
<body>
     ...
     */
    static createTenantTemplate(id, name, domain, primaryColor = '#f7931a') {
        return {
            id,
            name,
            domain,
...
export const EXAMPLE_TENANTS = {
    default: WhiteLabelManager.createTenantTemplate('default', 'Bitcoin Sovereign Academy', 'bitcoinsovereign.academy', '#f7931a'),
    school: WhiteLabelManager.createTenantTemplate('school', 'High School Bitcoin Program', 'bitcoin.school', '#3b82f6'),
    enterprise: WhiteLabelManager.createTenantTemplate('enterprise', 'Corporate Bitcoin Training', 'bitcoin.enterprise', '#8b5cf6'),
    government: WhiteLabelManager.createTenantTemplate('government', 'Policymaker Bitcoin Education', 'bitcoin.gov.example', '#059669')
};
