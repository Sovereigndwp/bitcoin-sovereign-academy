...
export class PersonaSelector {
    constructor() {
        ...
        this.storageKey = 'btc_academy_persona';
    }
    /**
     ...
     */
    getContentRecommendations(persona) {
     ...
     */
    customizeContent(content, persona) {
        const profile = this.getProfile(persona);
        if (!profile)
            return content;
        return {
...
`;
    ...
    adjustDifficulty(originalDifficulty, profile) {
        if (!profile.difficultyRange.includes(originalDifficulty)) {
            // Return closest difficulty in range
            return profile.difficultyRange[0];
        }
        return originalDifficulty;
    }
    adjustTimeEstimate(baseTime, commitment) {
        const multipliers = {
            low: 0.7,
            medium: 1.0,
            high: 1.3
