...
function isStageOneFreeModule(segment: string): boolean {
  return segment === 'module-1' || segment === 'module-1.html';
}

function isProtectedStageOneModule(segment: string): boolean {
  return segment.startsWith('module-') && !isStageOneFreeModule(segment);
}

export interface ProtectedRouteDetails {
  protected: boolean;
  pathname: string;
  area: 'deep-dive' | 'path' | null;
  pathId: string | null;
  reason: string;
}

export function getProtectedRouteDetails(inputPathname: string): ProtectedRouteDetails {
...
export function isProtectedPremiumRoute(inputPathname: string): boolean {
  return getProtectedRouteDetails(inputPathname).protected;
}

export function getPremiumPathIds(): string[] {
