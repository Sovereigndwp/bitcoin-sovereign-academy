...
A **pre-commit hook** (`.githooks/pre-commit`, tracked) runs the suite automatically and **blocks the commit** if it fails, but only when JEA files are staged. Enable once per clone: `git config core.hooksPath .githooks`.
...
- **Cleared in the fifth (estate/marital) batch:** Marital-property defaults verified — Colombia (sociedad conyugal), Switzerland (participation in acquisitions), Australia (no community property; Family Law Act 1975, contributions-based). **El Salvador testate forced-heirship is now a flagged CONFLICT** (absolute testamentary freedom vs forced heirs) — moved to open questions, public_safe false; intestate equal-share remains confirmed.
